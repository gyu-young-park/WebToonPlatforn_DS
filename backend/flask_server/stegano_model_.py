...
class SteganoGAN(object):
    ...
    def _coding_scores(self, cover, generated, payload, decoded):
        encoder_mse = mse_loss(generated, cover) # 여기 부분이 유사도 판단 부분인데 mse_loss 대신
        # L2 loss 를 활용할 수도 있는데..
    ...
    def fit(self, train, validate, epochs=5):
        ...
        for epoch in range(1, epochs + 1):
            ...
            '''
            shin++ 17:29/2020/10/20 start
            '''
            _val_E_mse = sum(METRIC_FIELDS['val.edncoder_mse'])/len(METRIC_FIELDS['val.edncoder_mse'])
            _val_D_loss = sum(METRIC_FIELDS['val.decoder_loss'])/len(METRIC_FIELDS['val.decoder_loss'])
            _val_D_acc = sum(METRIC_FIELDS['val.decoder_acc'])/len(METRIC_FIELDS['val.decoder_acc'])
            _val_Cov_score = sum(METRIC_FIELDS['val.cover_score'])/len(METRIC_FIELDS['val.cover_score'])
            _val_Gen_score = sum(METRIC_FIELDS['val.generated_score'])/len(METRIC_FIELDS['val.generated_score'])
            _val_ssim = sum(METRIC_FIELDS['val.ssim'])/len(METRIC_FIELDS['val.ssim'])
            _val_psnr = sum(METRIC_FIELDS['val.psnr'])/len(METRIC_FIELDS['val.psnr'])
            _val_bpp = sum(METRIC_FIELDS['val.bpp'])/len(METRIC_FIELDS['val.bpp'])

            _train_E_mse = sum(METRIC_FIELDS['train.encoder'])/len(METRIC_FIELDS['train.encoder'])
            _train_D_loss = sum(METRIC_FIELDS['train.decoder_loss'])/len(METRIC_FIELDS['train.decoder_loss'])
            _train_D_acc = sum(METRIC_FIELDS['train.decoder_acc'])/len(METRIC_FIELDS['train.decoder_acc'])
            _train_Cov_score = sum(METRIC_FIELDS['train.cover_score'])/len(METRIC_FIELDS['train.cover_score'])
            _train_gen_score = sum(METRIC_FIELDS['train.generated_score'])/len(METRIC_FIELDS['train.generated_score'])

            self.fit_metrics += 0.111*_val_E_mse
            self.fit_metrics += 0.111*_train_E_mse
            '''
            shin++ 17:29/2020/10/20 end
            '''
